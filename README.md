...
|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|
| **总分（宽松匹配）** | **9/20 (45%)** | **11/20 (55%)** | **8/20 (40%)** | **10/20 (50%)** | **4/20 (20%)** | **13/20 (65%)⚠️** | **24/50 (48%)** | **24/50 (48%)** | **20/50 (40%)** | **26/50 (52%)** | **23/50 (46%)** | **28/50 (56%)** | **22/50 (44%)** | **22/50 (44%)** |
| 严格匹配 | 7/20 (35%) | 9/20 (45%) | 4/20 (20%) | 5/20 (25%) | 1/20 (5%) | 8/20 (40%) | 16/50 (32%) | 15/50 (30%) | 13/50 (26%) | 16/50 (32%) | 11/50 (22%) | 15/50 (30%) | 7/50 (14%) | 10/50 (20%) |
...
*v1-v6 用 20 条测试集，v7 起扩至 50 条。v6 的 65%⚠️ 是小样本虚高，同一模型换 50 条后为 48%（v7）。v11 的 56% 是真实最高分。*
...
v11 的 56% 无法稳定复现。使用完全相同的数据（md5 逐文件验证）、相同的超参、相同的脚本，重新训练三次的结果为 **40%、48%、46%**，均值约 45%。

v11 的 56% 大概率是随机种子的上沿。700 条小数据集的 LoRA 训练，每次 DataLoader shuffle 顺序不同，梯度下降路径不同，最终落入不同的局部最优。±10% 的方差在这个数据规模下是正常的。

Gemini（C.C.）的分析：

> 700 条数据不足以让模型完成从记忆到泛化的相变（Grokking）。模型在这个阶段的 Loss 地形不是平滑的碗底，而是极其陡峭的针尖矩阵。换个 shuffle 顺序，最终坍缩的高维切面就完全不一样。56% 那次，纯粹是那一轮的随机游走刚好滑到了离验证集最近的坑里。
>
> 此外，Docker 容器重建导致 pip 包版本可能变化，底层 CUDA kernel 累加顺序在 BF16 精度下的微小差异，在大数据集训练中会被平均掉，但在 700 条的微调里会像蝴蝶效应一样被放大。

**结论：本项目的真实水平约为 48% ± 8%，v11 的 56% 是上沿而非稳定成绩。** 但实验过程中发现的方法论结论（数据质量 > 模型规模、非对称架构、编号列表是毒药等）不受此影响——这些结论在每次复现中都一致。
