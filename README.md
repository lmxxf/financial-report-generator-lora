...
| 指标 | v1 (14B/460) | v2 (14B/540) | v3 (32B) | v4 (32B lr↓) | v5 (32B lr↓↓) | v6 (14B/新TypeB) | v7 (14B/590/50测试) | v8 (14B/8epochs) | **v9 (全类型逐项排除)** |
|---|---|---|---|---|---|---|---|---|---|
| 完美匹配 ✅ | 7/20 (35%) | 9/20 (45%) | 4/20 (20%) | 5/20 (25%) | 1/20 (5%) | 8/20 (40%) | 16/50 (32%) | 15/50 (30%) | 13/50 (26%) |
| 宽松匹配 ✅🟡 | 9/20 (45%) | 11/20 (55%) | 8/20 (40%) | 10/20 (50%) | 4/20 (20%) | 13/20 (65%) | **24/50 (48%)** | **24/50 (48%)** | 20/50 (40%) |
| 单指标 | 4/5 | 4/5 | 1/5 | 2/5 | 1/5 | 5/5 | 9/10 (90%) | 8/10 (80%) | 8/10 (80%) |
| 边界判断 | 0/3 | 0/3 | 0/3 (🟡1) | 0/3 (🟡1) | 0/3 | 0/3 (🟡1) | 0/13 (🟡2) | 0/13 (🟡2) | 0/13 (🟡2) |
| JSON 合法率 | 20/20 | 20/20 | 20/20 | 20/20 | 20/20 | 20/20 | 50/50 | 50/50 | 50/50 |
| 空输出判断 | 2/2 | 2/2 | 2/2 | 2/2 | 0/2 | 2/2 | **5/5** | **5/5** | 4/5 |
| 类型准确率 | 95% | 100% | 100% | 91% | 52% | 100% | 98% | 98% | 96% |
| 预测总数/期望 | 39/32 | 40/32 | 44/32 | 42/32 | 70/32 | 39/32 | 106/72 | 106/72 | 102/72 |

*注：v1-v6 使用 20 条测试集，v7-v9 扩充至 50 条测试集，百分比不直接可比。*

**结论：**
1. **数据质量 > 模型规模 > 训练轮次 > 数据量。** 逐项排除式 analysis（v6）比换 32B 基座（v3-v5）更有效；加 epochs（v8）完全无效
2. **逐项排除是 Type B 的专用工具，不是万能格式。** v9 将全类型统一为逐项排除格式后退步（48%→40%）——简单场景不需要排除逻辑，强行排除反而诱导过度搜索
3. **不同数据类型需要不同的 analysis 风格。** Type A/C 适合简洁叙述，Type B 适合逐项排除，Type D 需要折中（排除+分类依据）
...
6. **已解决的场景：** 单指标（90%）、空输出（100%）、混合类型（80%）
7. **未解决的场景：** 边界判断（0/13 严格）、多指标（2/10）——14B 模型在"克制"和"精确数量控制"上有天花板
