...
├── test_cases_results_v1.json  # v1 评测详细结果（14B/460条/5epochs）
├── test_cases_results_v2.json  # v2 评测详细结果（14B/540条/5epochs）
├── test_cases_results_v3.json  # v3 评测详细结果（32B/540条/5epochs）
├── test_cases_results_v4.json  # v4 评测详细结果（32B/540条/5epochs/lr=5e-5/alpha=16）
├── test_cases_results_v5.json  # v5 评测详细结果（32B/540条/5epochs/lr=2e-5/alpha=16）
├── test_cases_results_v6.json  # v6 评测详细结果（14B/540条/5epochs/逐项排除式TypeB）
├── type_b_v2.jsonl             # C.C.改造后的Type B（逐项排除式analysis）
├── data/                      # 完整训练集（540 条，v2）
...
### v6：Qwen3-14B / 540 条 / 5 epochs / 逐项排除式 Type B（C.C. 改造）
...
### v5：Qwen3-32B / 540 条 / 5 epochs / lr=2e-5, alpha=16
...
| 指标 | v1 (14B/460条) | v2 (14B/540条) | v3 (32B) | v4 (32B lr↓alpha↓) | v5 (32B lr↓↓) | **v6 (14B/新TypeB)** |
|---|---|---|---|---|---|---|
| 完美匹配 ✅ | 7/20 (35%) | 9/20 (45%) | 4/20 (20%) | 5/20 (25%) | 1/20 (5%) | 8/20 (40%) |
| 宽松匹配 ✅🟡 | 9/20 (45%) | 11/20 (55%) | 8/20 (40%) | 10/20 (50%) | 4/20 (20%) | **13/20 (65%)** |
| 单指标 | 4/5 | 4/5 | 1/5 | 2/5 | 1/5 | **5/5** |
| 边界判断 | 0/3 | 0/3 | 0/3 (🟡1) | 0/3 (🟡1) | 0/3 | 0/3 (🟡1) |
| JSON 合法率 | 20/20 | 20/20 | 20/20 | 20/20 | 20/20 | 20/20 |
| 空输出判断 | 2/2 | 2/2 | 2/2 | 2/2 | 0/2 | 2/2 |
| 类型准确率 | 95% (21/22) | 100% (23/23) | 100% | 91% | 52% | 100% |
| 预测总数/期望 | 39/32 | 40/32 | 44/32 | 42/32 | 70/32 | 39/32 |

**结论：** 数据质量 > 模型规模 > 数据量。v6 证明改造 Type B 的 analysis（逐项排除式思维链）比换 32B 基座更有效。14B + 高质量数据（v6: 65%）完胜 32B + 相同数据（v4: 50%）。边界判断仍是最难的场景（0/3 严格），但已出现首个 🟡 突破口。
